...
public class ValidationServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try(PrintWriter out = response.getWriter()) {
            ...
            System.out.println(name + " " + category + " " + gender + " " + message);
            ...
            if (errors.size() > 0) {
                System.out.println("Errors galore" + errors);
                request.setAttribute("errors", errors);
                request.getRequestDispatcher("/contact-us/").forward(request, response);
            }
            else {
                System.out.println("No errors...");
                response.sendRedirect(request.getContextPath() + "/display?names=" + name + "&gender=" + gender + "&category=" + category + "&message=" + message);
            }
        }
    }
